...
class UserNotices(APIView):
    ...
    def get(self, request):
        ...
        # Retrieve the latest five unread notices for the authenticated user
        notices = Notice.objects.filter(user=user, is_read=False).order_by('-created_at')[:5]

        # Mark these notices as read
        notices.update(is_read=True)
...
class UpdateUserProfile(APIView):
    ...
    def put(self, request):
        ...
        user.contact_number = data.get('contact_number', user.guardian_name)
